...
class domain_list extends request
{
	...
	public function map()
	{
		...
		$dbs = db::query("
			SELECT	*
			FROM	domains
			ORDER BY lower(domain_name) DESC
			LIMIT	:num
			OFFSET	:offset
			;",
			array(
	...
	{
		echo self::$captions->get('domains');
	} # title()
	...
	public function html(&$args)
	{
		...
		echo '<table>'
			. '<thead>'
			. '<tr>'
				. '<th>' . self::$captions->get('domain_name') . '</th>'
		...
		foreach ( $this->data as $domain )
		{
			echo '<tr valign="top"' . ( ( $i % 2 ) ? ' class="alt"' : '' )  . '>'
				...
				. '<td align="center">'
					. '<a href="' . str::attr(permalink::get(array('cmd' => 'edit_domain', 'domain_id' => $domain->domain_id))) . '">'
	...
	public function list_pages()
	{
		...
		if ( $this->max_page > 1 )
		{
			...
			while ( ++$i <= $this->max_page )
			{
				...
				{
					echo '<a href="'
						. str::attr(permalink::get(array('cmd' => 'domains', 'page' => ( $i != 1 ? $i : null ))))
						. '">' . $i . '</a>';
				}
			}
		}
