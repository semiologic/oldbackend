...
class doc_cat_list extends request
{
	...
	public function map()
	{
		...
		$dbs = db::query("
			SELECT	*
			FROM	doc_cats
			ORDER BY lower(cat_name), lower(cat_version) DESC
			LIMIT	:num
			OFFSET	:offset
			;",
			array(
	...
	{
		echo self::$captions->get('doc_cats');
	} # title()
	...
	public function html(&$args)
	{
		...
		echo '<table>'
			. '<thead>'
			. '<tr>'
				. '<th>' . self::$captions->get('cat_name') . '</th>'
				. '<th style="width: 100px;">' . self::$captions->get('cat_version') . '</th>'
		...
		foreach ( $this->data as $cat )
		{
			echo '<tr valign="top"' . ( ( $i % 2 ) ? ' class="alt"' : '' )  . '>'
				. '<td>' . $cat->cat_name . '</td>'
				. '<td>' . $cat->cat_version . '</td>'
				. '<td align="center">'
					. '<a href="' . str::attr(permalink::get(array('cmd' => 'edit_doc_cat', 'cat_id' => $cat->cat_id))) . '">'
	...
	public function list_pages()
	{
		...
		if ( $this->max_page > 1 )
		{
			...
			while ( ++$i <= $this->max_page )
			{
				...
				{
					echo '<a href="'
						. str::attr(permalink::get(array('cmd' => 'doc_cats', 'page' => ( $i != 1 ? $i : null ))))
						. '">' . $i . '</a>';
				}
			}
		}
